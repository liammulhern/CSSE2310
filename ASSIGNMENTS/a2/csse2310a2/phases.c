...
void demo_one(char* input)
{
    ...
    append_to_secret_string(DEMO_ONE, 11 * num);
...
void phase_six(char* input)
{
    ...
    append_to_secret_string(6, (char)(48+PHASE_SIX_CONST*depth%8));
    depth+=1;
    if(depth/PHASE_SIX_CONST/28 == 0) {
	phase_six(input);
	return;
    }
    secret_string_matches(input);
...
void phase_seven(char* input)
{
    ...
    */
    ...
    append_to_secret_string(7, fortuneList->next->next->next->len + 75);
    for(int i = input[0]; i < input[0]+8; ++i) {
	if(i%2) {
	    fortuneList = fortuneList->next;
	    muteflip();
	    append_to_secret_string(7, i%2 + 75%2);
	}
    }
...
void phase_eight(char* input)
{
	...
	if(!(i%7)) { next_rchar();} else continue;
