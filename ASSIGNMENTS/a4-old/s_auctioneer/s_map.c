...
void map_insert_unsafe(Map* map, void* key, void* value) {
    ...
    size_t index = map_get_index(map, key);
    MapNode* currentNode = map->nodes[index];
    ...
    insertNode->next = map->nodes[index];
    map->nodes[index] = insertNode;
...
void map_remove(Map* map, void* key) {
    ...
    if (previousNode == NULL) {
        map->nodes[index] = currentNode->next;
    } else {
...
size_t map_get_index(Map* map, void* key) {
    size_t index = map_hash_function(key, HASH_SEED) % map->capacity;
    return index;
}
